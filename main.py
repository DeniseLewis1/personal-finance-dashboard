...
def main():
    ...
    # Explore data files
    #explore_data(transactions_df, categories_df)

    # Validate data files
    validate_categories(categories_df)
    ...
    for index, row in categories_df.iterrows():
        category_mapping[(row["name"], row["type"])] = row["id"]

    other_expense_id = category_mapping.get(("Other", "expense"))
    other_income_id = category_mapping.get(("Other Income", "income"))
    ...
    def get_category_id(row):
        ...
        if (category_name, type) in category_mapping:
            return category_mapping[(category_name, type)]
        
        else:
            return other_income_id if type == "income" else other_expense_id
